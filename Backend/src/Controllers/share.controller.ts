...
import { createShare, buildShareUrl, resolveShare, submitShareSection } from '../Services/share.service.js';
...
export const createShareHandler = asyncHandler(async (req: Request, res: Response) => {
  const share = await createShare(req.params['id'] as string, req.body.sectionId, req.body.role);
...
export const resolveShareHandler = asyncHandler(async (req: Request, res: Response) => {
  const share = await resolveShare(req.params['token'] as string);
