...
export function DocumentPreview({ pdfUrl }: { pdfUrl?: string }) {
  return (
        ...
        <div className="absolute inset-0 bg-background/20 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center rounded-lg">
          {pdfUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-surface-container-highest text-on-surface px-md py-sm rounded-full font-label-md text-label-md shadow-sm hover:shadow-md transition-all flex items-center gap-xs"
            >
              <Icon name="zoom_in" /> View Fullscreen
            </a>
          ) : (
            <span
              title="Generate your PDF first to preview it fullscreen"
              className="bg-surface-container-highest text-on-surface-variant px-md py-sm rounded-full font-label-md text-label-md shadow-sm flex items-center gap-xs cursor-not-allowed"
            >
              <Icon name="zoom_in" /> Generate PDF to preview
            </span>
          )}
        </div>
      </div>
